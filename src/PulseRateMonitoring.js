                ...
                {isLoading ? (
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {Array.isArray(pulseData) && pulseData.map(reading => (
                                <PulseCard key={reading.entry_id} reading={reading} />
                            ))}
                        </div>
...
function PulseCard({ reading }) {
    const pulseValue = reading.field1; 
    const isHighPulse = pulseValue > 300;

    return (
        <div className={`bg-gray-800 rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition duration-300 ease-in-out ${isHighPulse ? 'border border-red-500' : ''}`}>
            <div className="px-6 py-8">
                ...
                <p className="text-lg">Created At: {reading.created_at}</p>
